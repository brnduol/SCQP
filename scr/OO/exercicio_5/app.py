...
app = Flask(__name__)
UPLOAD_FOLDER = '/Users/figredos/Documents/unb/2024-1/OO/exercicio_5/upload'
...
class Usuario(db.Model):
    ...
    @staticmethod
    def verificar_credenciais(nome: str = None,
                              senha: str = None,
                              sign_up: bool = False) -> bool:
        if nome and senha:
...
def upload():
    if 'nome' in session:
        if request.method == 'POST':
            ...
            file.save(save_path)
            return render_template('upload.html', successful_upload=True)
...
def sign_up():
    if request.method == 'POST':
        ...
        if not Usuario.verificar_credenciais(
                nome=nome, senha=senha, sign_up=True):
